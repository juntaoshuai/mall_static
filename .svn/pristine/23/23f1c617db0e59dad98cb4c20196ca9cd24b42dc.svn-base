...
class Captcha {
	 ...
	 * @param string $qb 区别大小写（默认不区分大小写）
	 * @return bool 用户验证码是否正确
	 */
	public static function check($code, $qb=false, $id = '') {
//		源代码
//		isset($_SESSION) || session_start();
//		// 验证码不能为空
//		if(empty($code) || empty($_SESSION[self::$seKey])) return false;
//		$secode = $id ? $_SESSION[self::$seKey][$id] : $_SESSION[self::$seKey];
//		// session 过期
//		if(time() - $secode['time'] > self::$expire) return false;
//		if(strtoupper($code) == $secode['code']) return true;
//		return false;
		...
		if($id) {
			// session 过期
			if(time() - $_SESSION[self::$seKey][$id]['time'] > self::$expire) {
				unset($_SESSION[self::$seKey][$id]);
				return false;
			}
			if($qb){//区分大小写
				if($code == $_SESSION[self::$seKey][$id]['code']) {
					$_SESSION[self::$seKey][$id]['code'] = null;
					$_SESSION[self::$seKey][$id]['time'] = time() - (self::$expire + 1);//设为过期
					return true;
				}
			}else{//不区分大小写
				if(strtolower($code) == strtolower($_SESSION[self::$seKey][$id]['code'])) {
					$_SESSION[self::$seKey][$id]['code'] = null;
					$_SESSION[self::$seKey][$id]['time'] = time() - (self::$expire + 1);//设为过期
					return true;
				}
			...
			if($qb){//区分大小写
				if($code == $_SESSION[self::$seKey]['code']) {
					$_SESSION[self::$seKey]['code'] = null;
					$_SESSION[self::$seKey]['time'] = time()- (self::$expire + 1);//设为过期
					return true;
				}
			}else{//不区分大小写
				//echo $code.'----'.$_SESSION[self::$seKey]['code'] .'<br/>';
	...
	public static function entry($id = '') {
		...
		//$ttfPath = ROOTPATH . 'shared/plugins/Captcha/Captcha/' . (self::$useZh ? 'zhttfs' : 'ttfs') . '/';
		...
		for ($i = 0; $i<self::$length; $i++) {
			if(self::$useZh) {
				...
				$hh8 = false;
				if($code[$i] === 0 || $code[$i] === 1 || $code[$i] === 6 || $code[$i] === 9 || $code[$i] == 6 || $code[$i] == 9 || $code[$i] == 'o' || $code[$i] == 'O' || $code[$i]== i || $code[$i] == 'I' || $code[$i] == 'q' || $code[$i] == 'Q' || $code[$i] == 'c' || $code[$i] == 'C' || $code[$i] == 'l' || $code[$i] == 'r'){ $hh8 = true; }
				while ($hh8==true){
					$code[$i] = self::$_codeSet[mt_rand(0, 27)];
					if($code[$i] === 0 || $code[$i] === 1 || $code[$i] === 6 || $code[$i] === 9 || $code[$i] == 6 || $code[$i] == 9 || $code[$i] == 'o' || $code[$i] == 'O' || $code[$i]== i || $code[$i] == 'I' || $code[$i] == 'q' || $code[$i] == 'Q' || $code[$i] == 'c' || $code[$i] == 'C' || $code[$i] == 'l' || $code[$i] == 'r'){ $hh8 = true; }
					else{ $hh8 = false; }
		...
		if($id) {
			...
			//$_SESSION[self::$seKey][$id]['code'] = md5(join('', $code)); // 把校验码保存到session
			$_SESSION[self::$seKey][$id]['time'] = time();  // 验证码创建时间
		} else {
			$_SESSION[self::$seKey]['code'] = join('', $code); // 把校验码保存到session
			//$_SESSION[self::$seKey]['code'] = md5(join('', $code)); // 把校验码保存到session
	...
	private static function _background() {
		...
		//$path = ROOTPATH . 'shared/plugins/Captcha/Captcha/bgs/';
