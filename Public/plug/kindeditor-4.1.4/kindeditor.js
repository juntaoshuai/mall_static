...
function show_hide_code(id) {
    var code_id = "#code_" + id;
    var font_id = "#font_" + id;
    if ($(code_id) != undefined && $(font_id) != undefined) {
        var display_val = $(code_id).is(":hidden"); //是否隐藏，若是隐藏返回true，否则返回false
        if (display_val == true) {//若当前处于隐藏状态，则显示
            $(code_id).show(); //显示
            $(font_id).html("--");
        } else {//若当前处于显示状态，则隐藏
            $(code_id).hide(); //隐藏
...
function path() {
  ...
  ;
    ...
    for (var i = 0; i < args.length; i++)
        result.push(args[i].replace('@', 'http://www.mukeju.com/Public/Plug/kindeditor-4.1.4/syntaxhighlighter/scripts/'));
...
SyntaxHighlighter.defaults['toolbar'] = 'false'; /*去掉问号*/
//SyntaxHighlighter.defaults['collapse'] = 'true';/*一开始代码就折叠*/
SyntaxHighlighter.defaults['collapse'] = 'false'; /*一开始代码就不折叠（默认）*/
